...
if [[ $(pip show tensorflow-macos) == *tensorflow* ]] || [[ $(pip show tf-nightly) == *tf-nightly* ]] ; then
...
TF_CFLAGS=( $(python -c "import tensorflow as tf; print(' '.join(tf.sysconfig.get_compile_flags()))" | awk 'END{print $1}') )
TF_LFLAGS=( $(python -c "import tensorflow as tf; print(' '.join(tf.sysconfig.get_link_flags()))" | awk 'END{print $1}') )
TF_LFLAGS_2=( $(python -c "import tensorflow as tf; print(' '.join(tf.sysconfig.get_link_flags()))" | awk 'END{print $2}') )
TF_ABIFLAG=$(python -c "import tensorflow as tf; print(tf.sysconfig.CXX11_ABI_FLAG)" | awk 'END{print}')
